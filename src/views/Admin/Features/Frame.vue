<template>
  <div>
    ...
    <v-navigation-drawer width="300" v-model="drawer" app>
      <v-list-item to="/admin/features/main" color="accent">
        <v-list-item-content>
          <v-list-item-title>实用功能首页</v-list-item-title>
        </v-list-item-content>
      </v-list-item>
      <v-divider/>
      <v-list dense nav>
        <div v-for="r in routes" :key="r.key">
          <div v-if="r.meta">
            <v-list-item :to="'/admin/features/' + r.path" color="accent">
              <v-list-item-icon>
      ...
      <template v-slot:append>
        <v-bottom-navigation grow>
          ...
          <v-btn to="/admin/features" disabled><span> 实用功能 </span>
...
export default {
  ...
  computed: {
    title() {
      return this.$route.meta.title || "实用功能";
    },
    routes() {
  ...
  methods: {
    RoutePush(path) {
      if (this.$route.path !== "/features/" + path) this.$router.push("/features/" + path);
    },
  },
};
</script>
